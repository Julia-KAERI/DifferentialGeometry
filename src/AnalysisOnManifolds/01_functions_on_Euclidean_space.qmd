...
- $F:\R^m \to \R^n$ 에서 $\bf{x} = \begin{bmatrix} x^1 & \cdots & x^m \end{bmatrix}$ 에 대해 $F(\bf{x})$ 를 $F(x^1,\ldots,\,x^m)$ 으로 표기하기도 한다.
...
- 선형 변환 $L\in \mathcal{L}(\R^m,\,\R^n)$ 과 $\bf{x}= (x^1,\ldots,\,x^m)\in \R^m$ 에 대해 
  $$
  L(\bf{x}) = \sum_{i=1}^n \sum_{j=1}^m {L^i}_j x^j\bf{e}_i
  $$
  이다.
...
$\bf{h} = (h^1,\ldots,\,h^n)$ 이며 $(T(\bf{h}))^i = \sum_{j=1}^n {T^i}_j h^j$ 라 하자. 

$$
\left|\left(T(\bf{h})^i\right)\right| = \left| \sum_{j=1}^n {T^i}_{j}h^j\right| \le \sum_{j=1}^n |{T^i}_{j}|\cdot |h^j|
$$

이다. $M_0 = \max \{\left|{T^i}_{j}\right| : 1 \le i \le m,\, 1 \le j \le n\}$ 이라고 하자. 

$$
\begin{aligned}
\||T(\bf{h})\| & \le \sum_{i=1}^n \left|\left(T(\bf{h})\right)^i\right| \le \sum_{i=1}^m \sum_{j=1}^n |{T^i}_{j}|\cdot |h^j| \le \sum_{i, j}M_0 |h^j| \\[0.3em]
&\le \sum_{i, j}M_0 \|\bf{h}\|= mnM_0\|\bf{h}\|
\end{aligned}
$$
