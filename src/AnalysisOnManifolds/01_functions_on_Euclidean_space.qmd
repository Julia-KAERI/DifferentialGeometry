...
여기서는 유클리드 공간 $\R^n$ 에서의 해석학을 다룬다. 기본적인 정의와 성질에 대해서는 [유클리드 공간의 정의](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_1_basic_topology.html#def-MA_Euclidean_space) 와 [유클리드 노름에 대한 명제](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_1_basic_topology.html#prp-MA_norm_of_Euclidean_space) 를 보라. 

- 유클리드 공간 $\R^n$ 의 벡터 $\bf{x} = (x^1,\ldots,\,x^n)$ 는 $n \times 1$ 행렬 $\begin{bmatrix} x^1 & \cdots & x^n\end{bmatrix}^T$ 를 의미한다. 실제로 여기서는 $(x^1,\ldots,\,x^n)$ 식 표기법과 $\begin{bmatrix} x^1 & \cdots & x^n\end{bmatrix}^T$ 를 섞어서 사용한다.

- $F:\R^n \to \R^m$ 에서 $\bf{x} = (x^1,\ldots,\,x^n)$ 에 대해 $F(\bf{x})$ 를 $F(x^1,\ldots,\,x^n)$ 으로 표기하기도 한다.
...
- 선형 변환 $L\in \mathcal{L}(\R^m,\,\R^n)$ 과 $\bf{x}= (x^1,\ldots,\,x^m)\in \R^m$ 에 대해 
  $$
  L(\bf{x}) = \sum_{i=1}^n \sum_{j=1}^m L^i_j x^j\bf{e}_i
  $$
  
  이며

  $$
  L^i_j = \bf{e}_i \bf{\cdot} L(\bf{e}_j)
  $$

  이다. 

- $n \times n$ 실행렬 가운데 가역인 행렬의 집합을 일반선형군이라고 하며 $GL_n(\R)$ 이라고 표기한다. 
<br>
...
$\bf{x} = \sum_{i=1}^n x_i \bf{e}_i$ 이다. [유클리드 노름에 대한 명제](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_1_basic_topology.html#prp-MA_norm_of_Euclidean_space)  ($5$) 로 부터

$$
\|\bf{x}\| = \left\|\sum_{i=1}^n x^i \bf{e}_i\right\| \le |x^1| + \left\|\sum_{i=2}^n x^i \bf{e}_i\right\| \le \cdots \le \sum_{i=1}^n |x^i|
$$
...
[유클리드 노름에 대한 명제](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_1_basic_topology.html#prp-MA_norm_of_Euclidean_space)  ($5$) 에서 $\bf{y} \to \bf{y}-\bf{x}$ 로 바꾸면
...
선형 변환 $T \in \mathcal{L}(\R^n,\R^n)$ 이 $\|T(\bf{x})\|=\|\bf{x}\|$ 이면 **norm preserving** 이라고 하고 $\langle T(\bf{x}),\, T(\bf{y}) \rangle = \langle \bf{x},\,  \bf{y}\rangle$ 이면 **inner product preserving** 이라고 한다. 
...
선형변환 $T \in \mathcal{L}(\R^n, \R^m)$ 를 생각하자. 모든 $\boldsymbol{h}\in \R^n$ 에 대해 $\|T(\bf{h})\| \le M \|\bf{h}\|$ 인 $M$ 이 존재함을 보여라. 
...
$\bf{h} = (h^1,\ldots,\,h^n)$ 이며 $M_0 = \max \{\left|T^i_j\right| : 1 \le i \le m,\, 1 \le j \le n\}$ 이라고 하자. 

$$
\begin{aligned}
\| T(\bf{h})\| &  = \left\|\sum_{j=1}^\mathbf{text}\sum_{i=1}^n T(\bf{e}_i)h^i \bf{e}_j\right\| \le \left\|\sum_{j=1}^m\sum_{i=1}^n T(\bf{e}_i) \bf{e}_j \right\| \|\bf{h}\| \le nmM_0 \|\bf{h}\|
\end{aligned}
$$
...
$$
\begin{aligned}
T(\bf{x}_1+c\bf{x}_2) (\bf{y}) &= \varphi_{\bf{x}_1+c\bf{x}_2}(\bf{y})= (\bf{x}_1+c\bf{x}_2)\bf{\cdot y}= \bf{x}_1 \bf{\cdot y}+ c\bf{x}_2 \bf{\cdot y} \\[0.3em]
&= \varphi_{x_1}(\bf{y}) + c\varphi_{x_2}(\bf{y}) = \left(T(\bf{x}_1) + cT(\bf{x}_2)\right)(\bf{y})
\end{aligned}
$$
...
$\bf{x},\,\bf{y}\in\R^n$ 에 대해 $\bf{x \cdot y}=0$ 이면 $\bf{x}$ 와 $\bf{y}$ 는 서로 직교한다고 한다. $\bf{x},\,\bf{y}$ 가 직교한다면 $\|\bf{x}+\bf{y}\|^2 = \|\bf{x}\|^2+\|\bf{y}\|^2$ 임을 보여라. 
...
$\bf{x} = (x^1,\ldots,\,x^m)\in \R^m,\, \bf{y}=(y^1,\ldots,\,y^n) \in \R^n$ 에 대해 $(\bf{x},\,\bf{y})$ 를 다음과 같이 정의한다. 

$$
(\bf{x},\,\bf{y}) := (x^1,\ldots,\,x^m,\,y^1,\ldots,\,y^n) \in \R^{m+n}.
$$
...
[$n$-cell](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_1_basic_topology.html#def-MA_n_cell) 의 정의를 보라. 이와 비슷하게 

$$
(a_1,\,b_1) \times \cdots \times (a_n,\,b_n)\subset \R^n
$$

은 열린 집합이며 이를 **열린 $n$-cell** 이라고 한다. 또한
$$
[a_1,\,b_1] \times \cdots \times [a_n,\,b_n]\subset \R^n
$$

은 닫힌 집합이며 이를 **닫힌 $n$-cell** 이라고 한다.
...
$\bf{a} = (a^1,\ldots,\,a^n) \in \R^n$ 에 대해 $\bf{a}$ 를 중심으로 모서리의 길이가 모두 같은 열린 $n$-cell 을 아래와 같이 정의 할 수 있다. 

$$
\begin{aligned}
C(\bf{a},\,\delta) &:=  (a^1-\delta,\,a^1+\delta) \times \cdots \times (a^n - \delta,\, a^n+\delta), \\
\overline{C}(\bf{a},\,\delta) &:=  [a^1-\delta,\,a^1+\delta] \times \cdots \times [a^n - \delta,\, a^n+\delta]. 
\end{aligned}
$$
...
$A\in \R^n,\, \bf{x}\in \R^n$ 에 대해 

&emsp;($1$) $\bf{x}\in U\subset A$ 인 열린 $n$-cell $U$ 가 존재하면 $\bf{x}$ 가 $A$ 의 내부에 있다고 한다. 

&emsp;($2$) $\bf{x}$ 가 $\R^n\backslash A$ 의 내부에 있을 때 $\bf{x}$ 가 $A$ 의 외부에 있다고 한다.

&emsp;($3$) $\bf{x}$ 가 $A$ 의 내부와 외부에 있지 않을 때 $\bf{x}$ 가 $A$ 의 경계에 있다고 한다.
...
[기본적인 위상수학](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_1_basic_topology.html) 의 옹골집합 부분과 [하이네 보렐 정리](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_1_basic_topology.html#thm-MA_heine_borrel) 를 참고하라. 또한 [연습문제](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_1_basic_topology.html#exr-MA_KKK_2_7_20) 을 통해 $A\subset \R^m$ 과 $B\subset \R^n$ 이 옹골집합이면 $A \times B$ 도 옹골집합임을 보였다.
...
선형사상 $L\in \mathcal{L}(\R^n,\, \R^m)$ 에 대해 행렬의 **노름(norm)** $\|L\|$ 은 아래와 같이 정의된다.
...
행렬의 노름이 [거리 함수](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_1_basic_topology.html#def-MA_metric_space) 의 4가지 성질을 만족함을 보이면 되는데 이는 쉽게 보일 수 있다. $\square$

:::


$\mathcal{L}(\R^n,\, \R^m)$ 이 행렬의 노름에 대해 거리공간이므로 열림, 닫힘, 연속성과 같은 성질을 논할 수 있다. 아래의 성질은 이후 [역함수 정리와 음함수 정리](03_inverse_function_theorem.qmd) 에서 중요하게 사용된다.
...
$GL_n(\R)$ 은 $\R^n$ 에서의 모든 가역 선형 연자의 집합이다.

&emsp;($1$) $A\in GL_n(\R),\, B\in \mathcal{L}(\R^n)$ 이며 $\|B-A\|\cdot \|A^{-1}\| < 1$ 이면 $B \in GL_n(\R)$ 이다.

&emsp;($2$) $GL_n(\R)$ 는 $\mathcal{L}(\R^n)$ 의 열린 부분집합이며 $\phi(A)=A^{-1}$ 은 연속이다.
...
($b$) $A\in GL_n(\R)$ 에 대해 $\|B-A\|\le 1/\|A^{-1}\|$ 을 만족하는 $B\in \mathcal{L}(\R^n)$ 은 모두 가역이므로 $GL_n(\R)$ 은 열린집합이다. @eq-prp-CM_properties_of_matrix_norm_3_1 에서 $\bf{x}=B\bf{y}$ 라고 하면
...
### 대칭행렬의 성질 {#sec-CM_symmetric_metrix}

행렬 $A\in \R^{n \times n}$ 에 대해 $A = A^T$ 일 때 $A$ 를 대칭행렬이라고 한다. 여기서는 앞으로 다룰 대칭행렬의 여러가지 성질에 대해 알아보기로 하자. 다음 정리는 선형대수학을 참고하길 바라며 여기서 증명하지 않겠다.


</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-CM_diagonzibility_of_symmetric_metrix}

대칭행렬 $A\in \R^{n \times n}$ 는 모든 대각성분이 실수인 어떤 대각행렬과 닮았다. 어떤 직교행렬 $O$ 에 의해 

$$
\text{diag} (d_1,\ldots,\,d_n) = O^{T}AO
$$

이며 $O$ 의 $i$ 번째 행이 $d_i$ 를 고유값으로 갖는 $A$ 의 고유벡터이다.
...
다음 명제는 @thm-CM_diagonzibility_of_symmetric_metrix 로 부터 자명하다.
...
&emsp;($2$) $A$ 가 음부호 행렬이면 $\text{sgn}(\det (A)) = (-1)^n$ 이다. 
...
함수 $F:A\subset \R^n \to \R^m$ 은 $m$ 개의 **성분 함수(component function)** $f^i :\R^n \to \R$ $(i=1,\ldots,\,m)$ 를 가진다. 이 때 $F(\bf{x})$ 를 아래와 같이 열벡터로 표기한다.

$$
F(\bf{x}) =  \begin{bmatrix} f^1(\bf{x}) \\ \vdots \\ f^m(\bf{x}) \end{bmatrix}.
$$ 
...
벡터값 함수는 대문자 혹은 볼드체를 사용하며 성분함수는 일반체 소문자를 사용하기로 한다. 특별한 언급이 없다면 $F:\R^n \to \R^m$ 나 $\bf{f}:\R^n \to \R^m$ 에 대해 $f^i$ 는 성분함수이다.
...
항등함수 $\bf{\pi}:\R^n \to \R^n$ 은 $\bf{\pi}(\bf{x})=\bf{x}$ 이며 $\bf{\pi}$ 의 $i$ 번째 성분 함수를 $i$ 번째 **사영 함수(projection function)** 이라고 하고 $\pi_i$ 라고 쓴다. 즉 $\pi^i (\bf{x})= x^i$ 이다.
...
($2$) 사영함수 $\pi_i :\R^n \to \R$ 역시 선형사상이며 표준기저 $\bf{e}_i$ 에 대해 $\pi^i (\bf{x}) = \bf{e}_i^T \bf{\cdot x}$ 이다.  

($3$) $F = \begin{bmatrix} f^1 & \cdots & f^m\end{bmatrix}^T$ 일 때 $f^i = \pi^i (F)$ 이다.
...
연속성에 대해 [함수의 극한과 연속](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_3_continuity_of_functions.html#sec-MA_limit_and_continuity_of_functions) 을 참고하라.
...
$$
\begin{aligned}
M(f,\,\bf{a},\,\delta) &:= \sup \{f(\bf{x}): \bf{x}\in A,\, \|\bf{x}-\bf{a}\|<\delta\}, \\[0.4em]
m(f,\,\bf{a},\,\delta) &:= \inf \{f(\bf{x}): \bf{x}\in A,\, \|\bf{x}-\bf{a}\|<\delta\}
\end{aligned} 
$$

라 하자. 그리고 $\bf{a}$ 에서의 $f$ 의 **oscillation** $o(f,\bf{a})$ 를 다음과 같이 정의한다.

$$
\displaystyle o(f,\,\bf{a}) := \lim_{\delta \to 0} M(f,\,\bf{a},\,\delta) - m(f,\,\bf{a},\,\delta)
$$ 
...
$f$ 가 유계이므로 $M(f,\bf{a},\delta)$ 와 $m(f,\bf{a},\delta)$ 는 존재한다. $M(f,\bf{a},\delta)-m(f,\bf{a},\delta)$ 는 $\delta$ 에 대해 단조증가함수이므로 $o(f,\bf{a})$ 는 항상 존재한다.
...
$f$ 가 연속임을 가정하자. 임의의 $\varepsilon>0$ 에 대해 

$$
\|\bf{x}-\bf{a}\|<\delta \implies |f(\bf{x})-f(\bf{a})| < \varepsilon
$$ 

인 $\delta>0$ 이 존재한다. 따라서 $\|\bf{x}-\bf{a}\|<\delta$ 일 때
...
이제 $o(f,\,\bf{a})=0$ 임을 가정하자. Oscillation 의 정의에 의해 임의의 $\varepsilon >0$ 에 대해 어떤 $\delta_\bf{x}>0$ 이 존재하여 $M(f,\,\bf{a},\,\delta_\bf{x})- m(f,\, \bf{a},\,\delta_\bf{x})<\varepsilon$ 이다. $\|\bf{x}-\bf{a}\|<\delta_\bf{x}$ 이면 
...
$$
\displaystyle \lim_{\bf{x}\to \bf{a}} F(\bf{x}) = \bf{b} \iff \forall i\in \{1,\ldots,\,m\}\, \lim_{\bf{x}\to \bf{a}} f^i(\bf{x}) = b^i
$$
...
($\implies$) 임의의 $\varepsilon>0$ 에 대해 $0<\|\bf{x}-\bf{a}\|<\delta \implies \|F(\bf{x})-\bf{b}\|<\varepsilon$ 인 $\delta>0$ 이 존재한다. $|f^i(\bf{x})-b^i|\le\|f(\bf{x})-\bf{b}\|$ 이므로 성립한다. 


($\impliedby$) 임의의 $\varepsilon>0$ 에 대해 $0<\|\bf{x}-\bf{a}\|<\delta \implies |f_i(\bf{x})-b_i|<\varepsilon/m$ 인 $\delta>0$ 이 존재한다. 그렇다면.

$$
\|F(\bf{x})-\bf{b}\| \le \sum_{i=1}^m |f^i(\bf{x})-b^i| < m \cdot \dfrac{\varepsilon}{m} = \varepsilon
$$
...
see [최대최소정리](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_3_continuity_of_functions.html#thm-MA_extreme_value_theorem)
:::
...
$f:[a,\,b] \to \R$ 이 증가함수이다. $x_1,\ldots,\,x_n \in [a,\,b]$ 가 모두 다른 값이라면 $\sum_{i=1}^n o(f,\,x^i) < f(b)-f(a)$ 임을 보여라.
...
임의의 $\delta>0$ 에 대해 $o(f,\,x^i) < f(x^{i}+\delta)- f(x^i-\delta)$ 이다. 일반성을 잃지 않고 $x^1<\cdots <x^n$ 라고 할 수 있다. $\delta < \frac{1}{2}\min \{x^{k}-x^{k-1}: k=2,\ldots,\,n\}$ 으로 잡자. 그렇다면 $f(x^{k+1}-\delta) - f(x^k+\delta )>0$ 이며 

$$
\begin{aligned}
\sum_{i=1}^n o(f,\,x^i) &< f(x^1+\delta)- f(x^1-\delta) + \cdots + f(x^n+\delta)- f(x^n-\delta) \\
&= f(x^n+\delta) - (f(x^n-\delta)-f(x^{n-1}+\delta))  \\
&\qquad \qquad \cdots - (f(x^2-\delta)+ f(x^1+\delta)) - f(x^1-\delta)\\
&< f(x^n+\delta) - f(x^1-\delta)
\end{aligned}
$$

이다. 임의의 작은 $\delta>0$ 에 대해 성립하므로 $\sum_{i=1}^n o(f,\,x^i) < f(b)-f(a)$ 이다.
