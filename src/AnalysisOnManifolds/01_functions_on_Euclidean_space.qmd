...
- $F:\R^m \to \R^n$ 에서 $\bf{x} = (x^1,\ldots,\,x^m)$ 에 대해 $F(\bf{x})$ 를 $F(x^1,\ldots,\,x^m)$ 으로 표기하기도 한다.
...
$$
F(\bf{x}) = \begin{bmatrix} f^1(\bf{x}) & \cdots & f^m(\bf{x}) \end{bmatrix}^T= \begin{bmatrix} f^1(\bf{x}) \\ \vdots \\ f^m(\bf{x}) \end{bmatrix}.
$$ 
