...
- $\R$ 에서의 분할을 포함하여 1-변수 함수의 적분은 [리만-스틸체스 적분](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_5_riemann_stieltjes_integral.html) 를 참고하라.
...
짐작하듯이 $n$-cell $Q$ 에 대한 분할 $P$ 가 정해지면 이에 따른 모든 subcell 도 정해진다. 
...
$Q\subset \R^n$ 이 $n$-cell 이고 $Q$ 에 대해 정의된 함수 $f:Q \to \R$ 이 유계라고 하자. $P$ 가 $Q$ 의 분할이며 $S$ 가 $P$ 의 subcell 일 때 $M_S(f)$ 와 $m_S(f)$ 를 아래와 같이 정의한다.
...
$\text{vol}(S)$ 를 subcell $S$ 의 부피라고 할 때 분할 $P$ 에 대한 $f$ 의 **리만 상합** $U(f,\, P)$ 와 **리만 하합** $L(f,\,P)$ 는 다음과 같이 정의된다.
...
[일변수 함수에서의 분할과 관련된 정리](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_5_riemann_stieltjes_integral.html#thm-MA_properties_of_stieltjes_integral_1) 와 유사한 아래 정리는 아주 유용하다.
...
$n$-cell 에서 정의된 연속함수는 리만적분 가능함을 보여라.
...
$n$-cell $Q$ 는 옹골집합이며 $f :Q\to \R$ 이 연속이라면 $f$ 는 고른연속이다. 따라서 주어진 $\varepsilon >0$ 에 대해

$$
\|\bf{b}-\bf{a}\|< \delta \implies \|f(\bf{b})-f(\bf{a})\|<\dfrac{\varepsilon}{\text{vol}(Q)}
$$

가 되도록 하는 $\delta > 0$ 이 존재한다. 대각선의 길이가 $\delta$ 보다 작은 $n$-cell 로 이루어진 분할 $P\in \mathcal{P}[Q]$ 가 존재한다. 그렇다면

$$
U(f,\, P) - L(f,\, P) = \sum_{S\in P} (M_S-m_S)\text{vol}(S) < \sum_{S\in P} \dfrac{\varepsilon}{\text{vol}(Q)} \text{vol}(S) = \varepsilon
$$

이 성립한다. 임의의 $\varepsilon>0$ 에 대해 성립하므로 $f \in \mathcal{R}[Q]$ 이다. 
...
$$
\begin{gather}
m_S(f)+m_S(g) \le m_S(f+g),\\ 
M_S(f+g) \le M_S(f)+M_S(g)
\end{gather}
$$ {#eq-CM_spivak_3_3_1}
...
$$
\begin{aligned}
L(f,\,P) + L(g,\,P) &\le L(f+g,\,P) \\[0.3em]
&\le U(f+g,\,P) \le U(f,\,P) + U(g,\,P)
\end{aligned}
$$ {#eq-CM_spivak_3_3_2}
...
::: {#exm-CM_boundary_of_n_cell}

$\R^n$ 의 $n$-cell $X=[a_1,\,b_1]\times \cdots \times [a_n,\,b_n]$ 를 생각하자. 그리고

$$
m_c = (\max \{(b_i-a_i) : i=1,\ldots,\,n\})^{n-1}
$$ 

라고 하자. 정해진 $\varepsilon> 0$ 에 대해 $i$ 번째 축에 수직한 $n$-cell $X$ 의 면에 대해 $i$ 번째 차원을 제외한 나머지 두께는 $X$ 와 동일하고 $i$ 반째 방향의 두께가 $\varepsilon/(2nm_c)$ 가 되는 $n$-cell 을 생각 할 수 있다. 이 $n$-cell 은 $X$ 의 $i$ 번째 면을 포함하며 그 부피는 $\varepsilon/2n$ 보다 작다 . 이 $X$ 의 각 면을 포함하는 $2n$ 개의 $n$ cell 의 총 부피는 $\varepsilon$ 보다 작으므로 $n$ cell 의 경계의 부피는 measure zero 이다.
...
($a$) $n$-cell 의 집합 $\mathcal{U}=\{U_1,\ldots,\,U_m\}$ 가 $C$ 의 덮개이고 $U=\bigcup_{i=1}^n U_i$ 는 닫힌 집합이다. $C\subset U$ 이며 $\overline{C}=C\cup \partial C\subset U$ ([참고하라](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_1_basic_topology.html#prp-MA_closure)) 이므로 $\partial C\subset U$ 이다. 따라서 $C$ 의 경계도 content zero 이다.
...
$X=\{x: f\text{ is discontinuous at }x\}$ 라 하고, 정해진 자연수 $n$ 에 대해 $X_n = \{x:o(f,\,x)> \frac{1}{n}\}$ 라고 하자. @prp-CM_oscillation_and_continuity 로부터 $X_n \subset X$ 이다. 
...
$A$ 가 옹골집합이므로 유한개의 $U_{x_1},\ldots,\,U_{x_n}$ 이 존재하여 $A$ 를 덮는다. $A$ 의 분할 $P$ 를 $P$ 의 각각의 subcell 이 어떤 $U_{x_i}$ 에 포함되도록 잡을 수 있다. $S\in P$ 이면 $M_S(f)-m_S(f)<\varepsilon$ 이며, 따라서
...
닫힌 $n$-cell $A\subset \R^n$ 에서 정의된 유계인 함수 $f:A \to \R$ 을 생각하자. $B=\{\bf{x}\in A:f\text{ is not continuous at }\}$ 일 때 $f$ 가 적분가능한 것의 필요충분조건은 $B$ 가 measure zero 인 것이다. 
...
($1$) 우선 $B$ 가 measure zero 임을 가정하자 $\varepsilon>0$ 에 대해 $B_\varepsilon \subset B$ 이므로 $B_\varepsilon$ 은 measure zero 이다. @prp-CM_oscillation_and_closed_set 에 의해 $B_\varepsilon$ 은 닫힌집합이며 $B_\varepsilon \subset A$ 이므로 유계이다. 따라서 $B_\varepsilon$ 은 옹골집합이므로 content zero 이다(@prp-CM_content_zero_and_compact). 따라서 유한개의 닫힌 $n$-cell $U_1,\ldots,\,U_n$ 로 $\sum_{i=1}^n \text{vol}(U_i)<\varepsilon$ 이며 $B_\varepsilon$ 을 덮을 수 있다. 이제 $A$ 에 대한 분할 $P$ 를 $P$ 에 따라 정해지는 subcell $S$ 가 아래의 $\mathcal{S}_1$ 이나 $\mathcal{S}_2$ 중의 하나에 속하도록 잡자.(@fig-CM_integrability_1)

&emsp;($a$) $\mathcal{S}_1$ : 어떤 $U_i$ 에 대해 $S \subset U_i$ 이면 $S$ 는 $\mathcal{S}_1$ 에 포힘된다.

&emsp;($b$) $\mathcal{S}_2$ : $S\cap B_\varepsilon = \varnothing$ 이면 $S$ 는 $\mathcal{S}_2$ 에 포함된다.
...
이제 $S\in \mathcal{S}_2$ 이면 $o(f,\bf{x})<\varepsilon$ 이므로 @lem-CM_integrability 에 따라 $P$ 의 어떤 refinement $P'$ 이 존재하여 $S\in \mathcal{S}_2$ 일 때

$$
\sum_{S'} S [M_{S'}(f) - m_{S'}(f)]\cdot \text{vol}(S') < \varepsilon \cdot \text{vol}(S)
$$
...
::: {#cor-CM_integrability_and_measure_of_uncontinous_points}

$n$-cell $Q$ 에서 정의된 리만적분 가능함수 $f,\,g :Q \to \R$ 에 대해 다음이 성립한다.

&emsp;($1$) $fg\in \mathcal{R}[Q]$,

&emsp;($2$) $|f|\in \mathcal{R}[Q]$ 이며 아래 부등식이 성립한다.

$$
\left| \int_Q f\right| \le \int_Q |f|.
$$


&emsp;($3$) $\max \{f,\,g\} \in \mathcal{R}[Q],\, \min \{f,\,g\}\in \mathcal{R}[Q]$.
:::
...
$B_f,\, B_g$ 를 각각 $f,\,g$ 가 불연속인 위치라고 하자. 연속함수 $\phi : \R^2 \to \R$ 에 대해 $q : Q \to \R$ 을 $q(\bf{x}) := \phi (f(\bf{x}),\, g(\bf{x}))$ 라고 하자. $B_q$ 역시 $q$ 가 불연속적인 위치라고 하자. 그렇다면 $B_q \subset B_f \cup B_g$ 이며 따라서 $B_f,\,B_g$ 가 measure zero 이면 $B_q$ 도 measure zero 이고 $q \in \mathcal{R}[Q]$ 이다.

($1$) $\phi (x,\,y) = xy$ 는 연속함수이므로 $fg\in \mathcal{R}[Q]$ 이다.

($2$) $\phi(x,\,y) = |x|$ 는 연속함수이므로 $|f|\in \mathcal{R}[Q]$ 이다. $\left| \int_Q f \right|= c\int_Q f$ 을 만족하는 $c=\pm 1$ 이 존재한다. 이로부터

$$
\left| \int_Q f\right| = c \int_Q f = \int_Q cf \le \int_Q |f|
$$

($3$) $\phi = \max$ 혹은 $\phi = \min$ 은 연속함수 이므로 $\max \{f,\,g\} \in \mathcal{R}[Q],\, \min \{f,\,g\}\in \mathcal{R}[Q]$. $\square$
...
집합 $A$ 가 유계이고 $\partial A$ 가 measure $0$ 일 때 $A$ 를 **조르당 가측(Jordan-measurable)** 이라고 한다. $\int_A 1$ 을 $A$ 의 **content** 혹은 **$n$-차원 부피(volumn)** 라고 한다. $1$ 차원일 경우는 **길이(length)**, 2차원일 경우는 **넓이(area)** 라고 한다.
...
::: {.solution}
[단조함수의 연속성에 관한 명제](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_3_continuity_of_functions.html#prp-MA_monotonic_and_countable_number_of_discontinuity), @prp-CM_meazure_zero, @thm-CM_meazure_zero
...
$A,\,B \subset \R^n$ 가 조르당 가측 이면

$$
A \cup B,\quad A \cap B,\quad A\backslash B,\quad \overline{A},\quad \text{int}(A)
$$
...
$A,\,B$ 가 유계이므로 제시한 모든 집합은 유계이다. 따라서 제시한 집합의 경계가 measure 0 임을 보이면 된다. 

($1$) $\partial (A \cup B)\subset (\partial A \cup \partial B)$ 임을 보이이기 위해 $(\partial A \cup \partial B)^C \subset (\partial (A \cup B))^C$ 임을 보이자.

$x\not\in (\partial A \cup \partial B)$ 이면  $x$ 는 $A$ 의 내부 혹은 외부 그리고 $B$ 의 내부 혹은 외부 인 4가지 경우만 가능하며 모두 $x\notin (\partial (A \cup B))$ 이다. 따라서 $\partial (A \cup B)\subset (\partial A \cup \partial B)$ 이며 $\partial A \cup \partial B$ 가 measure 0 이므로 $\partial (A \cup B)$ 도 measure 0 이다.

($2$) $x\in \partial (A\cap B)$ 이면 

($3$) $A\backslash B = A \cap (B^C)$ 이며 $\partial (B^C)=\partial B$ 이므로 $B^C$ 역시 measure 0 이다. ($2$) 에 의해 $A\backslash B$ 는 measure 0 이다.

($4$) $\partial (\overline{A}) = \partial A$.

($5$) $\partial (\text{int}(A)) = \partial A$.
...
조르당 가측의 정의에 의해 $A$ 는 유계이며, $\partial A$ 역시 유계이다. [@KKK2011 연습문제 2.7.6](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_1_basic_topology.html#exr-MA_KKK_2_7_6) 에 의해 $\partial A$ 는 닫힌 집합며 따라서 [하이네 보렐 정리](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_1_basic_topology.html#thm-MA_heine_borrel) 에 따라 $\partial A$ 는 옹골집합이다. @prp-CM_content_zero_and_compact 에 따라 $\partial A$ 는 content $0$ 이다. 
:::
...
$g:\R \to \R$ 이 연속일 때 $\tilde{g}(x) := \int_a^x g$ 라고 하자. 그렇다면 [미적분의 제 1 기본정리](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_5_riemann_stieltjes_integral.html#thm-MA_first_fundamental_theorem_of_calculus) 에 의해 
