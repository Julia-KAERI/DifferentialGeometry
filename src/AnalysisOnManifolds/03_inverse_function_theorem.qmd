...
$$
f^i(\bf{y})-f^i(\bf{x}) = \nabla f^i(\bf{c}) (\bf{y}-\bf{x})
$$

를 만족하는 $\bf{c}$ 가 $\bf{y}$ 와 $\bf{x}$ 를 잇는 직선상에 존재한다. 
$$
\begin{aligned}
...
이제 $\bf{a}$ 를 중심으로 $U_1 \cap U_2 \cap U_3$ 에 포함되는 열린 $n$-입방체 $U$ 를 생각하자. $U$ 를 작게 잡아 $\partial U \cap \partial U_1 = \varnothing$ 이 되도록 잡을 수 있다. 이 때 $\partial U$ 는 $U$ 의 경계점(@KKK2011 연습문제 2.7.6) 이다. 
...
이 때 $W$ 는 열린집합이다(@Rudin1976 연습문제 4.3). 그렇다면 $\bf{y}\in W,\, \bf{x}\in \partial U$ 일 때
...
[연속함수와 역사상의 성질](https://julia-kaeri.github.io/MathematicalAnalysis/src/part2/2_3_continuity_of_functions.html#sec-MA_continuity_and_inverse_image) 로부터 $F:\R^n \to \R^m$ 에서 $F$ 가 연속가 연속인 것과 임의의 열린 집합 $W\subset \R^m$ 에 대해 $F^{-1}(W)$ 가 열린 집합인 것은 동치이다. 
