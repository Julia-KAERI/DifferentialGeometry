...
$F: \R^n \times \R^m \to \R^m$ 이 $(\bf{a}, \bf{b})$ 를 포함하는 열린 집합에서 $C^1$ 급 함수이며 $F(\bf{a}, \bf{b})=0$ 이라고 하자. $m \times m$ 행렬 $M$ 을 $M_{ij} = D_{n+j}f^i(\bf{a},\,\bf{b})$ 로 정할 때 $\det (M) \ne 0$ 이면 $\bf{a}$ 를 포함하는 어떤 열린 집합 $A\subset \R^n$ 와 $\bf{b}$ 를 포함하는 어떤 열린집합 $B\subset \R^m$ 에서 정의된 미분가능한 단사 함수 $G:A \to B$ 가 존재하여 $F(\bf{x},\, G(\bf{x})) = 0$ 이 되도록 한다.
...
$V$ 를 $A\subset \R^n,\, B\subset \R^m$ 에 대해 $V=A \times B$ 가 되도록 할 수 있으며$^1$[$^1$ 예를 들어 $n$ 입방체로 잡는다던지...]{.aside} 그렇다면 $\widetilde{F}^{-1}: W \to A \times B$ 는 어떤 $C^1$ 함수 $K:W \to B$ 에 대해 $\widetilde{F}^{-1}(\bf{x},\,\bf{y}) = (\bf{x},\, K(\bf{x},\, \bf{y}))$ 이다(@exm-CM_pts_of_inverse_function). 이제 $\Phi:\R^n \times \R^m \to \R^m$ 을 $\Phi (\bf{x},\, \bf{y}) := \bf{y}$ 로 정의하면 $\Phi \circ \widetilde{F} = F$ 이다. 따라서
...
$F(\bf{x},\,G(\bf{x}))=\bf{0}$ 이므로 $f^i(\bf{x},\, G(\bf{x}))=0$ 에 대해 양변에 $D_j$ 를 취하면 

$$
0 = D_j f^i(\bf{x},\, G(\bf{x})) + \sum_{k=1}^m D_{n+k}f^i (\bf{x},\, G(\bf{x})) \cdot D_j g^k(\bf{x})
$$

이다. ($F=\begin{bmatrix} f^1 & \cdots f^m\end{bmatrix}^T$ 이며 $G=\begin{bmatrix} g^1 & \cdots g^m\end{bmatrix}^T$ 이다.) 위 식은 행렬 $M_{ij} = D_{n+j}f^i(\bf{x},\, G(\bf{x}))$ 에 대한 선형방정식이며, $\det (M) \ne 0$ 이므로 $D_jg^k(\bf{x})$ 는 $D_jf^i(\bf{x},\, G(\bf{x}))$ 를 사용하여 구할 수 있다. $D_jg^k(\bf{x})$ 를 구할 수 있으므로 $G(\bf{x})$ 역시 구할 수 있다. 그러나 모든 편미분을 이용하여 구하는 것이므로 $G(\bf{x})$ 는 유일하지 않다. 그리고 일반적으로 매우 어렵다.
...
우선 @thm-CM_implicit_function_theorem_1 와의 차이를 보자. 양쪽 모두 $F'(\bf{a},\,\bf{b})$ 는 $m \times (n +m )$ 행렬이다. @thm-CM_implicit_function_theorem_1 은 정체 행과 마지막 $m$ 개의 열로 이루어진 행렬의 행렬식이 $0$ 이 아난 경우, 즉 마지막 $m$ 개의 열이 선형독립인 경우이며, @thm-CM_implicit_function_theorem_2 는 전체 $m+n$ 개의 열 가운데 위치에 무관하게 $m$ 개의 열이 선형독립인 경우이다. 
...
($3$) 귀납법을 이용해 증명한다. $k=1$ 경우는 이미 보였다. $k$ 에 대해 성립함을 가정하자. $F$ 가 $C^{k+1}$ 급이면 $F$ 는 $C^k$ 급이고 귀납적 가정에 의해 $G$ 역시 $C^k$ 급이다. 
$\square$
